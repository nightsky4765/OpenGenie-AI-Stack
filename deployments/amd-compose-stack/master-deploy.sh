#!/usr/bin/env bash
# =====================================================================
# TigerAI AMD-Compose-Stack Master Deployer
# Path: deployments/amd-compose-stack/master-deploy.sh
...
run_step() {
    ...
    if [ -d "$folder" ]; then
        ...
        if [ -f "./deploy.sh" ]; then
            ...
            LOG_WARN " $folder deploy.sh not found, running docker compose up..."
            docker compose up -d
        else
            LOG_WARN " ⚠️ No deploy.sh or docker-compose file found in $folder. Skipping Compose execution."
        fi
        popd > /dev/null
    else
...
case "$1" in
    ...
    all)
        ...
        ;;
    system)
        run_step "00-system-setup-rocm-docker"
